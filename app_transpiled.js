...
export function loadScatterPlotDataFromDB({ years, countries, genres }) {
...
async function refreshPlots({ years, countries, genres }, { refreshMap = true, refreshBarChart = true }) {
...
async function drawScatterPlot({ data, years }) {
  ...
  if (data.length === 0) {
    ...
    scatterPlot.append("text").attr("class", "placeholder").text("No data points to display").attr("x", svgWidth / 2).attr("y", svgHeight / 2).attr("text-anchor", "middle").attr("font-weight", 700);
    return;
  }

  const xScale = d3.scaleLinear().domain(d3.extent(data, d => d[xDataSelector])).range([padding, svgWidth - padding]);
  const yScale = d3.scaleLinear().domain(d3.extent(data, d => d[yDataSelector])).range([svgHeight - padding, padding]);
  const xAxis = d3.axisBottom(xScale).tickSize(-svgHeight + 2 * padding).tickSizeOuter(0).tickFormat(d => d3.format(".1s")(d));
  const yAxis = d3.axisLeft(yScale).tickSize(-svgWidth + 2 * padding).tickSizeOuter(0);
  colorScale = d3.scaleLinear().domain(d3.extent(data, d => d[cDataSelector])).range([d3.rgb("#66ff33"), d3.rgb("#cc0000")]);
  const radiusScale = d3.scaleLinear().domain(d3.extent(data, d => d[rDataSelector])).range([3, 12]);

  removeOldElements(scatterPlot);

  // Draw axes
  scatterPlot.append("g").call(xAxis).attr("transform", `translate (0, ${svgHeight - padding})`);
  ...
  // Axes labels
  // X axis
  scatterPlot.append("text").text(`${xLabel}. Circle size represents popularity`).attr("x", svgWidth / 2).attr("y", svgHeight - padding).attr("dy", "2em").attr("font-size", "1em").attr("text-anchor", "middle");

  // Y axis
  scatterPlot.append("text").text(yLabel).attr("transform", "rotate(-90)").attr("x", -svgHeight / 2).attr("y", 0).attr("dy", "0.7em").attr("font-size", "1em").attr("text-anchor", "middle");

  // Data points
  points.enter().append("circle").merge(points).attr("cx", d => d[xDataSelector] ? xScale(d[xDataSelector]) : padding).attr("cy", d => d[yDataSelector] ? yScale(d[yDataSelector]) : svgHeight - padding).attr("fill", "#444444").attr("r", d => radiusScale(d[rDataSelector]));

  points.raise();

  // tooltips
  const tooltip = d3.select(".tooltip");
  scatterPlot.selectAll("circle").on("mousemove", d => {
    tooltip.style("opacity", 1).html(`<b style="display: block">${d.title}</b>
        <img style="display: block" src="${imgBaseUrl + d.posterPath}" alt="" />`).style("left", `${d3.event.x /* - tooltip.node().offsetWidth/2 */ + 5}px`).style("top", `${d3.event.pageY}px`);
...
async function drawMap({ data }) {
  ...
  colorScale = d3.scaleLinear().domain(d3.extent(Array.from(countryCodeToMovieCount.values()))).range([colorbrewer.Blues[5][0], colorbrewer.Blues[5][4]]);

  const projection = d3.geoNaturalEarth1().scale(140).translate([svgWidth * 0.47, svgHeight * 0.58]);

  const path = d3.geoPath().projection(projection);

  const worldMap = d3.select("#worldMap").attr("width", svgWidth).attr("height", svgHeight).selectAll(".country").data(geoData);

  worldMap.exit().remove();

  const defaultCountryFillColor = "#DDDDDD";

  worldMap.enter().append("path").attr("class", "country").merge(worldMap).attr("d", path).attr("fill", d => {
  ...
  }).attr("stroke", d => countriesChosen.includes(codeNumericToLetter.get(d.id)) ? "#444444" : "blue").attr("stroke-width", d => countriesChosen.includes(codeNumericToLetter.get(d.id)) ? "1px" : "0");
  ...
  const tooltip = d3.select(".tooltip");
  d3.select("#worldMap").selectAll(".country").on("mousemove", handleMouseOver).on("mouseout", handleMouseOut).on("click", d => {
  ...
  function handleMouseOver(d) {
    ...
    tooltip.style("opacity", 1).text(codeNumericToName.get(d.id)).style("left", `${coords[0] - tooltip.node().offsetWidth / 2 + document.getElementById("worldMap").getBoundingClientRect().x}px`).style("top", `${coords[1] - 35 + document.getElementById("worldMap").getBoundingClientRect().y + window.pageYOffset}px`);
  }
  ...
  const legend = d3.select("#worldMap").append("defs").attr("width", legendWidth).attr("height", legendHeight).append("svg:linearGradient").attr("id", "gradient").attr("x1", "0%").attr("y1", "100%").attr("x2", "100%").attr("y2", "100%").attr("spreadMethod", "pad");
  ...
  d3.select("#worldMap").append("rect").attr("width", legendWidth).attr("height", legendHeight - 30).attr("x", `${svgWidth * 0.1}px`).attr("y", `${svgHeight * 0.95}px`).style("fill", "url(#gradient)");

  const y = d3.scaleLinear().range([0, legendWidth]).domain(d3.extent(Array.from(countryCodeToMovieCount.values())));

  const yAxis = d3.axisBottom().scale(y).ticks(5);

  d3.select("#worldMap").append("g").attr("class", "y axis").attr("transform", `translate(${svgWidth * 0.1}, ${svgHeight * 0.95})`).call(yAxis);
} // drawMap
...
function drawBarChart({ data }) {
  ...
  const barPadding = 3;
  const barWidth = Math.min((svgWidth - padding * 2) / Array.from(genreToMovieCount.values()).length - barPadding, svgWidth / 10);
  const bars = barChart.attr("width", svgWidth).attr("height", svgHeight).selectAll(".bar").data(genreData, d => d[0]);
  ...
  if (data.length === 0) {
    ...
    barChart.append("text").attr("class", "placeholder").text("No data points to display").attr("x", svgWidth / 2).attr("y", svgHeight / 2).attr("text-anchor", "middle").attr("font-weight", 700);
    return;
  }

  const paddingBottom = 10;
  const yScale = d3.scaleLinear().domain([0, d3.max(genreData, d => d[1])]).range([svgHeight, paddingBottom + padding]);

  const yAxis = d3.axisLeft(yScale).tickSize(padding / 3).tickFormat(e => Math.floor(e) === e ? e : undefined);
  // .tickSizeOuter(0);
  ...
  // Bars
  bars.enter().append("rect").attr("class", "bar").merge(bars).attr("x", (d, i) => padding + (barWidth + barPadding) * i).attr("y", d => yScale(d[1]) - paddingBottom).attr("height", d => svgHeight - yScale(d[1])).attr("width", barWidth).attr("fill", "#4B80B7").attr("opacity", 0.5).on("click", d => handleClick(d[0])).attr("stroke", d => genresChosen.includes(d[0]) ? "#222222" : "blue").attr("stroke-width", d => genresChosen.includes(d[0]) ? "2px" : "0");
  ...
  // Bar labels
  barLabels.enter().append("text").attr("class", "barLabel").merge(barLabels).text(d => d[0]).attr("y", (d, i) => padding + (barWidth + barPadding) * i + barWidth / 1.5).attr("x", -svgHeight + paddingBottom * 2).on("click", d => handleClick(d[0]));
...
function addLabel(selection, labelText) {
  selection.append("text").attr("class", "plotLabel").text(labelText).attr("x", svgWidth / 2).attr("y", 0).attr("dy", "0.75em").attr("font-size", "1.2em").attr("text-anchor", "middle");
}

function addNote(selection, noteText) {
  selection.append("text").text(noteText).attr("x", svgWidth / 2).attr("y", 0).attr("dy", "2.4em").attr("font-size", "0.7em").attr("text-anchor", "middle");
}
...
async function displayMovieInfo(id) {
  ...
  const runTime = movie.runtime < 60 ? `${movie.runtime} min` : `${Math.floor(movie.runtime / 60)} hr ${movie.runtime % 60} min`;
  let countries = "";
  ...
  const htmlString = `
  <img style="display: block; float:left; margin: 1%; height: auto; width: 256px;" src="${imgBaseUrlLarge + movie.posterPath}" alt="Image poster not found" />
